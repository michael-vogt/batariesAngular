      ...
      @if (mitglieder().length === 0) {
        ...
        <div class="karte">
          <table class="daten">
            ...
            <tbody>
              @for (zeile of zeilen(); track zeile.mitglied.id) {
                <tr>
                  <td>
                  ...
                  <td class="zahl" [class.betrag-offen]="zeile.finanzen.offeneForderungenGesamt > 0">
                  ...
                  <td>
                    @if (bearbeiteId() === zeile.mitglied.id) {
                      ...
                      <button type="button" class="leise-aktion" (click)="loeschen(zeile.mitglied)">
                        Entfernen
                      </button>
                    }
                  </td>
            ...
            <tfoot>
              <tr>
                ...
                <td class="zahl"><strong>{{ euro(summeOffen()) }}</strong></td>
                <td class="zahl"><strong>{{ euro(summeGuthaben()) }}</strong></td>
                <td></td>
              </tr>
            </tfoot>
          </table>
...
export class MitgliederListeComponent {
  ...
  protected readonly summeOffen = computed(() =>
    this.zeilen().reduce((s, z) => s + z.finanzen.offeneForderungenGesamt, 0),
  );
  protected readonly summeGuthaben = computed(() =>
    this.zeilen().reduce((s, z) => s + z.finanzen.restguthaben, 0),
  );
  ...
  protected anlegen(): void {
    ...
    if (this.mitglieder().some(m => m.name.toLowerCase() === name.toLowerCase())) {
      // Namen dienen weiterhin als Anzeigeschlüssel in Buchungstexten,
      // deshalb Dubletten früh abfangen.
      this.anlegeFehler.set(`„${name}“ ist bereits erfasst.`);
      return;
    }
